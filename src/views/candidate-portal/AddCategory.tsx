...
const AddCategory = () => {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({ defaultValues: { name: '', candidate_unique_id: '' } });

  useEffect(() => {
    if (!accessIds) return;
    candidatesService.publicGetAll({ size: 200, module_unique_id: accessIds.module_unique_id }).then(res => {
