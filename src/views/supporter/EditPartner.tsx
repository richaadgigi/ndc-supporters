...
const EditPartner = () => {
      ...
      .then(res => {
        if (res.success && res.data) {
          ...
          reset({
            ...
            state: p.state || '',
            lga: p.lga || '',
            where_do_you_operate: p.where_do_you_operate || '',
            what_does_organisation_do: p.what_does_organisation_do || '',
            what_would_you_want_to_do_together: p.what_would_you_want_to_do_together || '',
          });
          setCharterAreas(Array.isArray(p.charter_areas) ? p.charter_areas : []);
          setSocialLinks(Array.isArray(p.social_links) ? p.social_links : []);
        } else { setError(res.message || 'Failed to load partner'); showAlert('error-alert'); }
      })
      .catch(err => { setError(extractErrorMessage(err, 'Failed to load partner')); showAlert('error-alert'); })
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const response = await partnersService.editDetails({
        unique_id: id,
      ...
      if (response.success) {
        setSuccessMessage('Partner updated successfully'); showAlert('success-alert');
        setTimeout(() => router.push('/dashboard/supporter/partners'), 1500);
      } else { setError(response.message || 'Failed to update partner'); showAlert('error-alert'); }
    } catch (err: any) { setError(extractErrorMessage(err, 'Failed to update partner')); showAlert('error-alert'); } finally { setLoading(false); }
  };
  ...
  return (
    <div>
      <Navbar title="Edit Partner" subtitle="Update partnership details" />
