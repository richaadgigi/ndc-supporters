...
const AddMember = () => {
  ...
  const { register, handleSubmit, reset, watch, setValue, control, formState: { errors } } = useForm<FormData>({
    defaultValues: {
      ...
      gender: '', date_of_birth: '', code: '', nin: '', support_group_unique_id: '', member_role_unique_id: '',
    },
  });
    ...
    const loadDropdowns = async () => {
      try {
        const [cRes, rRes] = await Promise.all([
          supportGroupsService.getAll({ size: 200, ...params }),
          memberRolesService.getAll({ size: 100, ...params }),
        ]);
        if (cRes.success && cRes.data) setSupportGroups(Array.isArray(cRes.data) ? cRes.data : (cRes.data as any).rows || []);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        reset();
        setTimeout(() => router.push('/dashboard/supporter/members'), 1500);
      } else {
            ...
            <div>
              ...
              <div className="xui-form-box" {...(errors.support_group_unique_id && { 'xui-error': 'true' })}>
                ...
                <select id="support_group_unique_id" {...register('support_group_unique_id', { required: 'Support group is required' })}>
                  ...
                  {supportGroups.map((g) => (
                    <option key={g.unique_id} value={g.unique_id}>{g.name}{g.scope_option ? ` (${g.scope_option})` : ''}{g.state ? ` - ${g.state}` : ''}</option>
                  ))}
                </select>
