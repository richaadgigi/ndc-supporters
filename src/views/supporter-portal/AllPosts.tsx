...
import { Renew, Edit, TrashCan, Add, Download, Time, UserAvatar, View, Checkmark } from '@carbon/icons-react';
...
const AllPosts = () => {
  ...
  return (
    <div>
      <Navbar title="Posts" subtitle="Manage support group posts" />
      <div className="xui-py-1-half">
        <div className="xui-d-flex xui-flex-ai-center xui-flex-jc-space-between xui-mb-1-half" style={{ flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half">
            ...
            {canAdd && (
              <button onClick={() => router.push('/dashboard/supporter-portal/posts/add')} className="xui-btn xui-font-sz-80 xui-bdr-rad-half xui-font-w-500 xui-d-flex xui-flex-ai-center xui-grid-gap-half" style={{ backgroundColor: 'var(--primary-600)', color: 'var(--secondary-700)' }}>
            ...
            <div className="xui-d-grid xui-grid-col-1 xui-md-grid-col-2 xui-lg-grid-col-3 xui-grid-gap-1">
              {items.map((item) => (
                <div
                  ...
                  onClick={() => router.push(`/dashboard/supporter-portal/posts/view/${item.unique_id}`)}
                ...
                >
                  <div className="xui-w-fluid-100 xui-overflow-hidden xui-pos-relative" style={{ height: '200px', backgroundColor: 'var(--neutral-100)' }}>
                    ...
                    {item.Category && (
                      <span className="xui-font-sz-70 xui-font-w-600" style={{ position: 'absolute', top: '12px', left: '12px', backgroundColor: 'var(--primary-600)', color: 'var(--secondary-700)', padding: '4px 10px', borderRadius: '4px', letterSpacing: '0.3px', textTransform: 'uppercase' }}>
                        {item.Category.name}
                      </span>
                    )}
                  ...
                  <div className="xui-p-1">
                    ...
                    <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-1 xui-mb-1 xui-flex-wrap">
                      ...
                      <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half" style={{ color: 'var(--neutral-400)' }}>
                        <View size={14} />
                        <span className="xui-font-sz-70">{item.views?.toLocaleString() || 0}</span>
                      </div>
                    </div>

                    {(canEdit || canDelete) && (
                      <div className="xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-pt-1" style={{ borderTop: '1px solid var(--neutral-100)' }}>
                        {canEdit && !item.approved_by && (
                          <button onClick={(e) => { e.stopPropagation(); setSelectedItem(item); modalShow('approve-modal'); }} className="xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-cursor-pointer xui-font-sz-70 xui-font-w-500" style={{ backgroundColor: 'var(--success-light)', border: 'none', color: 'var(--success)', padding: '6px 12px', borderRadius: '4px' }}>
                            <Checkmark size={14} /> Approve
                          </button>
                        )}
                        {canEdit && !item.approved_by && (
                          <button onClick={(e) => { e.stopPropagation(); router.push(`/dashboard/supporter-portal/posts/edit/${item.unique_id}`); }} className="xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-cursor-pointer xui-font-sz-70 xui-font-w-500" style={{ backgroundColor: 'var(--info-light)', border: 'none', color: 'var(--info)', padding: '6px 12px', borderRadius: '4px' }}>
                            <Edit size={14} /> Edit
                          </button>
                        )}
                        {canDelete && (
                          <button onClick={(e) => { e.stopPropagation(); setSelectedItem(item); modalShow('delete-modal'); }} className="xui-d-flex xui-flex-ai-center xui-grid-gap-half xui-cursor-pointer xui-font-sz-70 xui-font-w-500" style={{ backgroundColor: 'var(--error-light)', border: 'none', color: 'var(--error)', padding: '6px 12px', borderRadius: '4px' }}>
